...
void LowPrio::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.SimLog.LowPrio" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "LowPrio", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "LowPrio" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.Comment", get_Comment_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.FedName", get_FedName_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.Time", get_Time_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.actualLogicalGenerationTime", get_actualLogicalGenerationTime_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.federateFilter", get_federateFilter_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.originFed", get_originFed_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "LowPrio.sourceFed", get_sourceFed_handle() )  );
...
void LowPrio::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void LowPrio::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void LowPrio::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void LowPrio::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool LowPrio::static_init( void ) {
	...
	getClassNameSet().insert( "LowPrio" );

	getClassNameFactoryMap().insert(  std::make_pair( "LowPrio", &LowPrio::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "LowPrio", (PubsubFunctionPtr)( &LowPrio::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "LowPrio", (PubsubFunctionPtr)( &LowPrio::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "LowPrio", (PubsubFunctionPtr)( &LowPrio::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "LowPrio", (PubsubFunctionPtr)( &LowPrio::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "LowPrio", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "LowPrio", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, LowPrio::SP entitySP ) {
	return os << *entitySP;
}
std::ostream &operator<<( std::ostream &os, const LowPrio &entity ) {
	return os << "LowPrio("
