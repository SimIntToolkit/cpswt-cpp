...
void SimPause::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.SimulationControl.SimPause" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "SimPause", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "SimPause" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimPause.actualLogicalGenerationTime", get_actualLogicalGenerationTime_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimPause.federateFilter", get_federateFilter_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimPause.originFed", get_originFed_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimPause.sourceFed", get_sourceFed_handle() )  );
...
void SimPause::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void SimPause::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void SimPause::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void SimPause::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool SimPause::static_init( void ) {
	static bool isInitialized = false;
	if ( isInitialized ) {
		return true;
	}
	...
	getClassNameSet().insert( "SimPause" );

	getClassNameFactoryMap().insert(  std::make_pair( "SimPause", &SimPause::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "SimPause", (PubsubFunctionPtr)( &SimPause::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "SimPause", (PubsubFunctionPtr)( &SimPause::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "SimPause", (PubsubFunctionPtr)( &SimPause::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "SimPause", (PubsubFunctionPtr)( &SimPause::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimPause", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimPause", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, SimPause::SP entitySP ) {
	return os << *entitySP;
}
std::ostream &operator<<( std::ostream &os, const SimPause &entity ) {
	return os << "SimPause("
