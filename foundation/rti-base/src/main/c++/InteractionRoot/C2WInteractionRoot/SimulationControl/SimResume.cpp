...
void SimResume::init( RTI::RTIambassador *rti ) {
	...
	while( isNotInitialized ) {
		try {
			getHandle() = rti->getInteractionClassHandle( "InteractionRoot.C2WInteractionRoot.SimulationControl.SimResume" );
	...
	getClassNameHandleMap().insert(  std::make_pair( "SimResume", get_handle() )  );
	getClassHandleNameMap().insert(  std::make_pair( get_handle(), "SimResume" )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimResume.actualLogicalGenerationTime", get_actualLogicalGenerationTime_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimResume.federateFilter", get_federateFilter_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimResume.originFed", get_originFed_handle() )  );
	...
	getDatamemberNameHandleMap().insert(  std::make_pair( "SimResume.sourceFed", get_sourceFed_handle() )  );
...
void SimResume::publish( RTI::RTIambassador *rti ) {
	if ( getIsPublished() ) {
		return;
	}
...
void SimResume::unpublish( RTI::RTIambassador *rti ) {
	if ( !getIsPublished() ) {
		return;
	}
...
void SimResume::subscribe( RTI::RTIambassador *rti ) {
	if ( getIsSubscribed() ) {
		return;
	}
...
void SimResume::unsubscribe( RTI::RTIambassador *rti ) {
	if ( !getIsSubscribed() ) {
		return;
	}
...
bool SimResume::static_init( void ) {
	static bool isInitialized = false;
	if ( isInitialized ) {
		return true;
	}
	...
	getClassNameSet().insert( "SimResume" );

	getClassNameFactoryMap().insert(  std::make_pair( "SimResume", &SimResume::factory )  );
	getClassNamePublishMap().insert(   std::make_pair(  "SimResume", (PubsubFunctionPtr)( &SimResume::publish )  )   );
	getClassNameUnpublishMap().insert(   std::make_pair(  "SimResume", (PubsubFunctionPtr)( &SimResume::unpublish )  )   );
	getClassNameSubscribeMap().insert(   std::make_pair(  "SimResume", (PubsubFunctionPtr)( &SimResume::subscribe )  )   );
	getClassNameUnsubscribeMap().insert(   std::make_pair(  "SimResume", (PubsubFunctionPtr)( &SimResume::unsubscribe )  )   );

	getDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimResume", &getDatamemberNames() )  );
	getAllDatamemberClassNameVectorPtrMap().insert(  std::make_pair( "SimResume", &getAllDatamemberNames() )  );
...
std::ostream &operator<<( std::ostream &os, SimResume::SP entitySP ) {
	return os << *entitySP;
}
std::ostream &operator<<( std::ostream &os, const SimResume &entity ) {
	return os << "SimResume("
