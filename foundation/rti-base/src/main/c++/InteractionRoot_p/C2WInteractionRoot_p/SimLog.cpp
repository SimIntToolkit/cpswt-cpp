...
bool SimLog::static_init() {
    ...
    get_class_name_instance_sp_map()[get_hla_class_name()] = SP(new SimLog());
...
int SimLog::get_parameter_handle_aux(const std::string &className, const std::string &propertyName) {
...
void SimLog::init(RTI::RTIambassador *rti) {
...
//            logger.error(e);
    ...
    while(isNotInitialized) {
        try {
            ...
            propertyHandle = rti->getParameterHandle("Comment", get_class_handle());
            classAndPropertyNameSP = ClassAndPropertyNameSP( new ClassAndPropertyName(
              get_hla_class_name(), "Comment"
            ) );
            ...
            propertyHandle = rti->getParameterHandle("FedName", get_class_handle());
            classAndPropertyNameSP = ClassAndPropertyNameSP( new ClassAndPropertyName(
              get_hla_class_name(), "FedName"
            ) );
            get_class_and_property_name_handle_map()[*classAndPropertyNameSP] = propertyHandle;
            get_handle_class_and_property_name_sp_map()[propertyHandle] = classAndPropertyNameSP;

            propertyHandle = rti->getParameterHandle("Time", get_class_handle());
            classAndPropertyNameSP = ClassAndPropertyNameSP( new ClassAndPropertyName(
              get_hla_class_name(), "Time"
            ) );
            get_class_and_property_name_handle_map()[*classAndPropertyNameSP] = propertyHandle;
            get_handle_class_and_property_name_sp_map()[propertyHandle] = classAndPropertyNameSP;
...
void SimLog::publish_interaction(RTI::RTIambassador *rti) {
    if (get_is_published()) {
        return;
    }
...
void SimLog::unpublish_interaction(RTI::RTIambassador *rti) {
...
void SimLog::subscribe_interaction(RTI::RTIambassador *rti) {
...
void SimLog::unsubscribe_interaction(RTI::RTIambassador *rti) {
...
SimLog::PropertyClassNameAndValueSP SimLog::getParameterAux(
