...
DOCKER_OMNET_WORKSPACE_DIR=/home/omnetpp/workspace
HOST_PROJECT_DIR="$(realpath "$1")"
DOCKER_PROJECT_DIR="$DOCKER_OMNET_WORKSPACE_DIR/Project"


touch "$HOST_XAUTH"
xauth nlist :0 | sed -e 's/^..../ffff/' | xauth -f "$HOST_XAUTH" nmerge -
docker run -it --rm -v "$XSOCK":"$XSOCK" -v "$HOST_XAUTH":"$DOCKER_XAUTH" -v "$HOST_PROJECT_DIR:$DOCKER_PROJECT_DIR" \
